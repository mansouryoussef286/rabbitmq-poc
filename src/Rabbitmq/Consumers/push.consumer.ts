...
export class PushConsumer {
	private queueName: string = 'notifications.push';
	...
	async Listen() {
		this.channelWrapper =
			await this.rabbitMQService.createandGetChannelWrapper('Push');
		...
		console.log(
			`👂 Push Consumer listening on '${this.queueName}' with tag ${this.consumerTag}`,
		);
	}
