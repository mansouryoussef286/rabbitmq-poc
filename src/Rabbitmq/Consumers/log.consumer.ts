...
export class LogConsumer {
	private queueName: string = 'service.logger';
	...
	async Listen() {
		this.channelWrapper =
			await this.rabbitMQService.createandGetChannelWrapper('logs');
		...
		console.log(
			`👂 Log Consumer listening on '${this.queueName}' with tag ${this.consumerTag}`,
		);
	}
